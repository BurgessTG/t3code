...
import { EventId, IsoDateTime, NonNegativeInt, TrimmedNonEmptyString } from "./baseSchemas.ts";
...
export const PatchTarget = Schema.Struct({
  ...
  pathGlobs: Schema.Array(TrimmedNonEmptyString).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
  notes: Schema.NullOr(TrimmedNonEmptyString).pipe(
    Schema.withDecodingDefault(Effect.succeed(null)),
  ),
  priority: NonNegativeInt.pipe(Schema.withDecodingDefault(Effect.succeed(0))),
});
...
export const PatchProfile = Schema.Struct({
  ...
  targets: Schema.Array(PatchTarget).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
});
...
export const PatchRunSummary = Schema.Struct({
  ...
  applyBranch: Schema.NullOr(TrimmedNonEmptyString).pipe(
    Schema.withDecodingDefault(Effect.succeed(null)),
  ),
...
export const PatchRun = Schema.Struct({
  ...
  conflicts: Schema.Array(PatchConflict).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
  validationResults: Schema.Array(PatchValidationResult).pipe(
    Schema.withDecodingDefault(Effect.succeed([])),
  ),
  logs: Schema.Array(PatchLogEntry).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
});
