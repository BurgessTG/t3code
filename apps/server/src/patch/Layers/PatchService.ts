...
function splitRepositoryName(repository: string): { owner: string; repo: string } {
  const [owner = "", repo = ""] = repository.split("/", 2);
  return { owner, repo };
}

function parseGitRemoteRepository(remoteUrl: string): string | null {
  ...
  const genericMatch =
    /^(?:ssh|https?|git):\/\/[^/]+\/([^/\s]+\/[^/\s]+?)(?:\.git)?\/?$/i.exec(trimmed);
...
function makeValidationResultId(runId: PatchRun["id"], command: string, index: number) {
  return PatchValidationResultId.make(
...
function makeLogEntry(
  ...
  return {
    id: EventId.make(`patch-log-${randomUUID()}`),
    patchRunId: runId,
    level,
...
const makePatchService = Effect.gen(function* () {
    ...
    Effect.gen(function* () {
      ...
      for (const [index, command] of commands.entries()) {
        ...
        const outputExcerpt = [processResult.stdout.trim(), processResult.stderr.trim()]
          ...
          .join("\n\n")
          .slice(0, 4_000);
        const processExitCode = processResult.code === null ? null : Number(processResult.code);
        const passed = !processResult.timedOut && processExitCode === 0;
  ...
  const reconcile: PatchServiceShape["reconcile"] = (input, options) =>
    Effect.gen(function* () {
      ...
      yield* runGit("PatchService.reconcile.createWorktree", repoContext.repoRoot, [
        "worktree",
        "add",
        "-b",
        sandboxBranch,
      ...
      run = {
        ...
        logs: [
          ...run.logs,
          makeLogEntry(run.id, "info", `Created sandbox worktree at ${sandboxPath}.`),
        ],
      };
      yield* persistRun(run);
      yield* emit(publisher, {
        ...
        createdAt: nowIso(),
        phase: "sandbox",
        message: `Created sandbox worktree at ${sandboxPath}.`,
      });
  ...
  const apply: PatchServiceShape["apply"] = (input, options) =>
    Effect.gen(function* () {
      ...
      const statusDetails = yield* runGit(
      ...
      const checkedOut =
        statusDetails.stdout.trim().length === 0 && currentHead === run.forkHeadBefore;
      if (checkedOut) {
        yield* runGit("PatchService.apply.checkout", profile.repoRoot, ["checkout", applyBranch]);
      }
  ...
  const discardRun: PatchServiceShape["discardRun"] = (input, options) =>
    Effect.gen(function* () {
      ...
      if (sandboxExists) {
        yield* runGit("PatchService.discardRun.removeWorktree", profile.repoRoot, [
          "worktree",
          "remove",
          "--force",
          run.sandboxWorktreePath,
