...
type PatchToastId = ReturnType<typeof toastManager.add>;
...
export default function PatchControl({ gitCwd, activeThreadId }: PatchControlProps) {
  ...
  const runToastIdsRef = useRef(new Map<string, PatchToastId>());
  ...
  const statusQuery = useQuery(patchStatusQueryOptions(gitCwd));
  ...
  const generateProfileMutation = useMutation(
    patchGenerateProfileMutationOptions({ cwd: gitCwd, queryClient }),
  );
  ...
  const applyMutation = useMutation(patchApplyMutationOptions({ cwd: gitCwd, queryClient }));
  const openSandboxMutation = useMutation(patchOpenSandboxMutationOptions({ cwd: gitCwd }));
  const discardRunMutation = useMutation(
    patchDiscardRunMutationOptions({ cwd: gitCwd, queryClient }),
  );
  ...
  const closeRunToast = useCallback((runId: string) => {
    const toastId = runToastIdsRef.current.get(runId);
    if (!toastId) return;
    toastManager.close(toastId);
    runToastIdsRef.current.delete(runId);
  }, []);

  const handlePatchEvent = useEffectEvent((event: PatchEvent) => {
    if (gitCwd && event.cwd !== gitCwd) return;

    switch (event.kind) {
      case "run_started": {
        closeRunToast(event.runId);
        const toastId = toastManager.add({
          type: "loading",
          title: "Reconciling patch layer",
          description: event.message,
          timeout: 0,
          data: threadToastData,
        });
        runToastIdsRef.current.set(event.runId, toastId);
        void invalidatePatchQueries(queryClient);
        return;
      }
      case "run_progress": {
        const toastId = runToastIdsRef.current.get(event.runId);
        if (!toastId) return;
      ...
      case "conflict_detected":
        toastManager.add({
          ...
          description: event.conflict.filePath,
          data: threadToastData,
        });
        void invalidatePatchQueries(queryClient);
        return;
      case "validation_completed":
        if (event.result.status === "failed") {
          toastManager.add({
            ...
            description: event.result.command,
            data: threadToastData,
          });
        }
        void invalidatePatchQueries(queryClient);
      ...
      case "run_finished":
        closeRunToast(event.runId);
        toastManager.add({
          type: event.status === "ready_for_review" ? "success" : "info",
      ...
      case "apply_completed":
        ...
        toastManager.add({
          ...
          description: event.checkedOut
            ? `Checked out ${event.applyBranch}.`
            : `Created ${event.applyBranch}.`,
          data: threadToastData,
        });
        void invalidatePatchQueries(queryClient);
        return;
  ...
  useEffect(() => {
    ...
    if (!api) return;
    return api.patch.onEvent((event) => {
