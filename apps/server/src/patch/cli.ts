  ...
  return Effect.gen(function* () {
    ...
    switch (command) {
      ...
      case "get-run": {
        ...
        const result = yield* patchService.getRun({ cwd, runId: PatchRunId.makeUnsafe(runId) });
        yield* Effect.sync(() => printJson(result));
        return;
      }
      ...
      case "apply": {
        ...
        const result = yield* patchService.apply({ cwd, runId: PatchRunId.makeUnsafe(runId) });
        yield* Effect.sync(() => printJson(result));
        return;
      }
      ...
      case "open-sandbox": {
        ...
        const result = yield* patchService.openSandbox({
          cwd,
          runId: PatchRunId.makeUnsafe(runId),
        });
        yield* Effect.sync(() => printJson(result));
        return;
      }
